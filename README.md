...
An end-to-end time series forecasting case study built on daily unit-sales data from the Corporación Favorita grocery chain in Ecuador. The project covers the full ML workflow: exploratory analysis, statistical and ML model training, hyperparameter optimisation, experiment tracking, and an interactive Streamlit dashboard.
...
> **VS Code shortcut:** Press `Cmd+Shift+B` (Mac) or `Ctrl+Shift+B` (Windows/Linux) to launch the dashboard directly from the editor, with no terminal typing needed. A `tasks.json` is included in `.vscode/` that wires this up automatically.

---

## Live Dashboard: four pages
...
| **HyperOpt Results** | Baseline vs tuned delta table, grouped bar charts, best hyperparameters for all 4 models |
...
│
├── notebooks/
│   ├── 01_eda.ipynb                         # Phase 1: EDA and data cleaning
...
│   ├── 04_hyperparameter_tuning_and_mlflow.ipynb   # Phase 4: HyperOpt and MLflow
...
│   └── timeseries_features.csv     # Phase 1 output: cleaned and feature-engineered
│
├── models/
...
│
├── images/                         # All figures: fig_01 to fig_07 EDA (Phase 1), phase2a_*
│                                   #   statistical (Phase 2), phase3_* ML (Phase 3),
│                                   #   fig_08 to fig_11 tuning (Phase 4), plus dashboard.png
│                                   #   and residuals.png
│
...
│
└── .vscode/
    └── tasks.json                  # Cmd/Ctrl+Shift+B runs streamlit run app.py
```
...
Daily unit-sales for one product line, Jan 2013 to Mar 2014 (454 observations). Train / test split: train up to 2013-12-31, test 2014 Q1 (90 days). Key EDA findings:

- **Weekly seasonality (s = 7):** Saturday +33%, Sunday +45% vs weekday average
- **Stationary series:** ADF test p < 0.05, so no differencing required
...
All models evaluated on the same 90-day held-out test window (Jan to Mar 2014):

| Model | MAE | RMSE | MAPE (%) | R² |
|---|---|---|---|---|
| **Prophet (baseline), champion** | **94.9** | **142.8** | **21.0** | **0.437** |
...
HyperOpt, from 94.9 to 113.2, while XGBoost improved from 115.0 to 105.9 and still
lost to every statistical model. Giving each model an identical 50-trial budget is
what makes that visible: had only XGBoost been tuned, it would have looked competitive.
...
about -10.6, and the same negative bias appears across every model in the project
rather than in one of them. The residual distribution is roughly symmetric, so there
is no directional drift over the test period, just a consistent downward offset. The
cause is the same one behind XGBoost's poor showing: roughly 335 usable training rows
after lag trimming is simply not much history to learn a level from.
...
MSIT Data Analytics programme, Berlin.
